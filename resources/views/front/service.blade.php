														...
														<ul>
															@foreach($subCategory->services as $service)
																<li>
																	...
																	<span class="txt">{{$service->title}}: </span><span class="service_amount"> ₹ {{$service->price}}</span>
																	<span class="add_cart_btn">
																		<button type="button" class="add_to_cart" data-type="service" data-id="{{ $service->id }}" data-title="{{ $service->title }}" data-price="{{ $service->price }}">Add to Cart</button>
																	</span>
																</li>
															@endforeach
														</ul>
...
<div class="pricing_plan_section" id="packages">
	<div class="container">
		<div class="inner_container">
			<div class="salon_services_img">
									...
									<div class="inner_box">
										...
										@if(!empty($package->services) && count($package->services) > 0)
											...
											<div class="continue_btn">
												<button type="button" class="add_to_cart" data-type="package" data-id="{{ $package->id }}" data-price="{{ $package->price }}" data-title="{{ $package->title }}"
													data-description="<ul>
													@foreach($package->services as $service)
														<li>{{$service->title}}</li>
													@endforeach
